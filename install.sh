...
t() {
  ...
  case "$LANG_CHOICE:$key" in
    ru:ca_mode_prompt) echo "Режим работы Step-CA" ;;
    en:ca_mode_prompt) echo "Step-CA operational mode" ;;
    ru:ca_mode_bundled) echo "Bundled (Step-CA в Docker-контейнере)" ;;
    en:ca_mode_bundled) echo "Bundled (Step-CA in Docker container)" ;;
    ru:ca_mode_external) echo "External (внешний/нативный Step-CA на хосте или удалённом сервере)" ;;
...
wait_all_services() {
  local url="$1" all_healthy=true code ca_mode
  ca_mode="$(get_env_value CA_MODE)"
  [[ -n "$ca_mode" ]] || ca_mode="bundled"
  say_step "$(t wait_ready)"
  local services=(postgres step-ui)
  if [[ "$ca_mode" == "bundled" ]]; then
    services=(postgres step-ca step-ui)
  fi
...
install_mode() {
  ...
  printf "  %s: ${C_BOLD}%s${C_RESET}\n" "$(t timezone)" "$tz_value"
  _log "HOST_IP=$host_ip UI_HTTPS_PORT=$ui_port CA_MODE=$ca_mode PROVISIONER=$provisioner TZ=$tz_value"
...
ensure_update_env() {
	...
	local host_ip ui_port tz_value provisioner ca_mode compose_file
	...
	[[ -n "$compose_file" ]] || compose_file="docker-compose.yml:docker-compose.bundled.yml"

	append_missing_env CA_MODE "$ca_mode"
	append_missing_env COMPOSE_FILE "$compose_file"
